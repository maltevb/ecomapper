...
def main(train_config, caption_config):
    ...
    logger.info("Training dataset preprocessing complete.")
    ...
    transformer = SD3Transformer2DModel.from_pretrained(
        train_config.pretrained_single_image_transformer_path, subfolder="transformer", torch_dtype=torch.float16
    )
    ...
    if train_config.controlnet_model_name_or_path:
        ...
        controlnet = SD3ControlNetModel.from_pretrained(
            train_config.controlnet_model_name_or_path
        )
    else:
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                ...
                load_model = SD3ControlNetModel.from_pretrained(
    ...
    for epoch in range(first_epoch, train_config.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                pixel_values = batch["img"].to(dtype=vae.dtype, device=accelerator.device, non_blocking=True)
    ...
    if accelerator.is_main_process:
        ...
        controlnet.save_pretrained(train_config.output_dir)
